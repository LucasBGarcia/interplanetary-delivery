...
export function Edit() {
    ...
    const [EnderecoEdit, setEnderecoEdit] = useState<FormProps>();


    useEffect(() => {
        const response = localStorage.getItem('cadastros')
        if (response) {
            const EnderecosParse = JSON.parse(response)
            setEnderecos(EnderecosParse)
            const enderecoEncontrado = EnderecosParse.find((endereco: { id: number | undefined; }) => endereco.id == id);
            if (enderecoEncontrado) {
                setEnderecoEdit(enderecoEncontrado);
                setCarregando(false);
            } else {
                setCarregando(false);
            }
                ...
                <div className="flex flex-col md:flex-row gap-3">
                    {Carregando ?
                        <div className=" flex flex-col flex-1 p-4 items-center justify-center rounded-md border-gray-950 gap-3">
                            <svg aria-hidden="true" className="inline w-40 h-40 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className="text-black">carregando Form...</span>
                        </div>
                        :
                        <EditCadastro dados={EnderecoEdit} />
                    }
                    {Carregando ?
                        <div className=" flex flex-col flex-1 p-4  items-center justify-center rounded-md border-gray-950 gap-3">
                            <svg aria-hidden="true" className="inline w-40 h-40 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
